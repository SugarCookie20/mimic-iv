CREATE OR REPLACE TABLE `my-mimic-research.my_results.aki_cohort_final_definition_outcomes` AS
(
WITH
Base_Cohort AS (
  SELECT
  ...
  FROM (
      SELECT
          ...
          adm.admittime,
          icu.stay_id,
          ROW_NUMBER() OVER (PARTITION BY icu.subject_id ORDER BY icu.intime ASC) as icu_stay_rank
      ...
      WHERE
          icu.los > 1
          AND (EXTRACT(YEAR FROM adm.admittime) - pat.anchor_year + pat.anchor_age) >= 18
          AND EXISTS (
              SELECT 1 FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` d
              WHERE d.hadm_id = icu.hadm_id
                AND ((d.icd_version = 9 AND (d.icd_code LIKE '584%' OR d.icd_code = '586'))
                     OR (d.icd_version = 10 AND (d.icd_code LIKE 'N17%' OR d.icd_code = 'N19')))
          )
  ) AS ranked_stays
...
First_ICU_Stay AS (
  SELECT
      ...
      los AS icu_los_days
  FROM (
       SELECT hadm_id, stay_id, intime, outtime, los,
       ROW_NUMBER() OVER (PARTITION BY hadm_id ORDER BY intime ASC) as rn
       FROM `physionet-data.mimiciv_3_1_icu.icustays`
  ) AS ranked_icustays
  WHERE hadm_id IN (SELECT hadm_id FROM Base_Cohort) AND rn = 1
),
Relevant_AKI_Codes AS (SELECT hadm_id, STRING_AGG(icd_code, ', ') AS aki_icd_code FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` d WHERE (d.icd_version = 9 AND (d.icd_code LIKE '584%' OR d.icd_code = '586')) OR (d.icd_version = 10 AND (d.icd_code LIKE 'N17%' OR d.icd_code = 'N19')) GROUP BY hadm_id),
Pregnancy_Codes AS (SELECT hadm_id, STRING_AGG(icd_code, ', ') AS pregnancy_icd_codes FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` WHERE (icd_version = 10 AND REGEXP_CONTAINS(icd_code, r'^Z3[3469]')) OR (icd_version = 9 AND REGEXP_CONTAINS(icd_code, r'^V2[234]')) GROUP BY hadm_id),
Congenital_Codes AS (SELECT hadm_id, STRING_AGG(icd_code, ', ') AS congenital_anomaly_icd_codes FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` WHERE (icd_version = 9 AND REGEXP_CONTAINS(icd_code, r'^(74|75)')) OR (icd_version = 10 AND REGEXP_CONTAINS(icd_code, r'^Q')) GROUP BY hadm_id),
...
Malignancy_Codes AS (SELECT hadm_id, STRING_AGG(icd_code, ', ') AS malignancy_icd_codes FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` WHERE (icd_version = 9 AND REGEXP_CONTAINS(icd_code, r'^(1[4-9]|20)')) OR (icd_version = 10 AND REGEXP_CONTAINS(icd_code, r'^C')) GROUP BY hadm_id),
AIDS_Codes AS (
...
DNI_DNR_Codes AS (SELECT stay_id, STRING_AGG(DISTINCT value, '; ') AS dni_dnr_status FROM `physionet-data.mimiciv_3_1_icu.chartevents` WHERE itemid = 223758 AND value IN ('DNI (do not intubate)', 'DNR (do not resuscitate)', 'DNI/DNR') GROUP BY stay_id),

Comorbidities_Flags AS (
SELECT hadm_id,
    MAX(CASE WHEN (icd_version = 9 AND icd_code IN ('25001', '25003', '25011', '25013', '25021', '25023', '25031', '25033', '25041', '25043', '25051', '25053', '25061', '25063', '25071', '25081', '25083', '25091', '25093')) OR (icd_version = 10 AND icd_code LIKE 'E10%') THEN 1 ELSE 0 END) AS has_t1d,
    MAX(CASE WHEN (icd_version = 9 AND icd_code IN ('25000', '25002', '25010', '25012', '25020', '25022', '25030', '25032', '25040', '25042', '25050', '25052', '25060', '25062', '25070', '25072', '25080', '25082', '25090', '25092')) OR (icd_version = 10 AND icd_code LIKE 'E11%') THEN 1 ELSE 0 END) AS has_t2d,
    ...
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '410%' OR icd_code LIKE '411%' OR icd_code LIKE '412%' OR icd_code LIKE '413%' OR icd_code LIKE '414%')) OR (icd_version = 10 AND (icd_code LIKE 'I20%' OR icd_code LIKE 'I21%' OR icd_code LIKE 'I22%' OR icd_code LIKE 'I23%' OR icd_code LIKE 'I24%' OR icd_code LIKE 'I25%')) THEN 1 ELSE 0 END) AS has_ischemic_heart_disease,
    ...
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '410%' OR icd_code LIKE '412%')) OR (icd_version = 10 AND (icd_code LIKE 'I21%' OR icd_code LIKE 'I22%' OR icd_code LIKE 'I23%' OR icd_code LIKE 'I25%')) THEN 1 ELSE 0 END) AS has_myocardial_infarction,
    MAX(CASE WHEN (icd_version = 9 AND icd_code LIKE '428%') OR (icd_version = 10 AND (icd_code LIKE 'I50%' OR icd_code IN ('I110', 'I13', 'I9713', 'I0981'))) THEN 1 ELSE 0 END) AS has_heart_failure,
    ...
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '745%' OR icd_code LIKE '746%')) OR (icd_version = 10 AND (icd_code LIKE 'Q20%' OR icd_code LIKE 'Q21%' OR icd_code LIKE 'Q22%' OR icd_code LIKE 'Q23%' OR icd_code LIKE 'Q24%')) THEN 1 ELSE 0 END) AS has_congenital_heart_defect,
    ...
    MAX(CASE
    WHEN (icd_version = 9 AND icd_code IN ('45340', '45342', '45350', '45352'))
    OR (icd_version = 10 AND REGEXP_CONTAINS(icd_code, r'^(I8240|I8243|I8244|I8245|I8246|I8249|I824Z|I8250|I8253|I8254|I8255|I8256|I8259|I825Z)'))
    THEN 1
    ELSE 0
END) AS has_dvt,
    ...
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '481%' OR icd_code LIKE '482%')) OR (icd_version = 10 AND (icd_code LIKE 'J13%' OR icd_code LIKE 'J14%' OR icd_code LIKE 'J15%')) THEN 1 ELSE 0 END) AS has_bacterial_pneumonia,
    ...
    MAX(CASE
    WHEN (icd_version = 9 AND (icd_code LIKE '491%' OR icd_code LIKE '492%' OR icd_code LIKE '496%'))
...
END) AS has_copd,
    ...
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '512%' OR icd_code IN ('0117', '860'))) OR (icd_version = 10 AND (icd_code LIKE 'J93%' OR icd_code IN ('J86', 'J95811', 'S270', 'A15'))) THEN 1 ELSE 0 END) AS has_pneumothorax,
    MAX(CASE WHEN (icd_version = 9 AND icd_code LIKE '4160%') OR (icd_version = 10 AND (icd_code IN ('I270') OR icd_code LIKE 'I272%')) THEN 1 ELSE 0 END) AS has_pulmonary_hypertension,
    MAX(CASE WHEN icd_code = 'J80' THEN 1 ELSE 0 END) AS has_ards,
    MAX(CASE WHEN (icd_version = 9 AND icd_code IN ('51851', '51853', '51881', '51884', '51883', '51882')) OR (icd_version = 10 AND (icd_code IN ('J95822', 'J95821') OR icd_code LIKE 'J96%')) THEN 1 ELSE 0 END) AS has_respiratory_failure,
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '038%' OR icd_code IN ('0202', '0223'))) OR (icd_version = 10 AND (icd_code LIKE 'A40%' OR icd_code LIKE 'A41%' OR icd_code IN ('O85', 'A427', 'B377', 'A267', 'A282', 'A5486', 'A327', 'A392', 'A393', 'A394', 'A207', 'A217', 'A483', 'A227'))) THEN 1 ELSE 0 END) AS has_sepsis,
    MAX(CASE WHEN (icd_version = 9 AND icd_code IN ('5715', '5712', '5716')) OR (icd_version = 10 AND (icd_code LIKE 'K703%' OR icd_code LIKE 'K721%' OR icd_code IN ('K702', 'K704', 'K740', 'K741', 'K742', 'K743', 'K744', 'K745', 'K746'))) THEN 1 ELSE 0 END) AS has_cirrhosis,
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '070%' OR icd_code IN ('5731', '5732', '5733'))) OR (icd_version = 10 AND (REGEXP_CONTAINS(icd_code, r'^B1[5-9]') OR icd_code LIKE 'K701%' OR icd_code LIKE 'K73%' OR icd_code IN ('K754', 'K7581'))) THEN 1 ELSE 0 END) AS has_hepatitis,
    MAX(CASE WHEN (icd_version = 9 AND (icd_code LIKE '531%' OR icd_code LIKE '532%' OR icd_code LIKE '533%' OR icd_code LIKE '534%')) OR (icd_version = 10 AND (icd_code LIKE 'K25%' OR icd_code LIKE 'K26%' OR icd_code LIKE 'K27%' OR icd_code LIKE 'K28%')) THEN 1 ELSE 0 END) AS has_peptic_ulcer_disease,
    MAX(CASE WHEN (icd_version = 9 AND icd_code IN ('5851', '5852', '5853', '5854', '5855', '5856', '5859', '28521', '403', '404')) OR (icd_version = 10 AND (icd_code LIKE 'E102%' OR icd_code LIKE 'E112%' OR icd_code LIKE 'E132%' OR icd_code LIKE 'I12%' OR icd_code LIKE 'I13%' OR icd_code LIKE 'N18%' OR icd_code LIKE 'N03%' OR icd_code LIKE 'N04%' OR icd_code LIKE 'N05%' OR icd_code LIKE 'N11%' OR icd_code IN ('I150', 'I151', 'N19', 'N12', 'N14', 'N15', 'N16', 'N00', 'N01', 'N02', 'N06', 'N07', 'N08', 'M3214', 'M3215', 'M3504', 'M350A'))) THEN 1 ELSE 0 END) AS has_ckd,
    ...
    MAX(CASE WHEN (icd_version = 9 AND icd_code LIKE '290%') OR (icd_version = 10 AND (icd_code LIKE 'F01%' OR icd_code LIKE 'F02%' OR icd_code LIKE 'F03%' OR icd_code LIKE 'F04%')) THEN 1 ELSE 0 END) AS has_dementia,
    ...
    MAX(CASE WHEN (icd_version = 9 AND REGEXP_CONTAINS(icd_code, r'^20[0-2]')) OR (icd_version = 10 AND (icd_code LIKE 'C81%' OR icd_code LIKE 'C82%' OR icd_code LIKE 'C83%' OR icd_code LIKE 'C84%' OR icd_code LIKE 'C85%' OR icd_code LIKE 'C86%' OR icd_code LIKE 'C88%')) THEN 1 ELSE 0 END) AS has_lymphoma,
    ...
    MAX(CASE WHEN (icd_version = 9 AND icd_code LIKE '714%') OR (icd_version = 10 AND (icd_code LIKE 'M05%' OR icd_code LIKE 'M06%')) THEN 1 ELSE 0 END) AS has_rheumatoid_arthritis
FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` GROUP BY hadm_id
),
Patient_History AS (
  WITH All_Hemo_Events AS (
      SELECT subject_id, CAST(chartdate AS DATETIME) AS event_time FROM `physionet-data.mimiciv_3_1_hosp.procedures_icd`
      WHERE icd_code IN ('3995', '3927', '3943', '3895', '3942', '5A1D70Z', '5A1D80Z', '5A1D90Z')
      UNION ALL
      SELECT i.subject_id, ce.charttime AS event_time FROM `physionet-data.mimiciv_3_1_icu.chartevents` ce
      JOIN `physionet-data.mimiciv_3_1_icu.icustays` i ON ce.stay_id = i.stay_id
      WHERE ce.itemid IN (226118, 227357, 225725, 226499, 224154, 225183, 227438, 224191, 228004, 228005, 228006, 224144, 224145, 224149, 224150, 224151, 224152, 224153)
      UNION ALL

      SELECT adm.subject_id, adm.admittime AS event_time
      FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` d
      JOIN `physionet-data.mimiciv_3_1_hosp.admissions` adm ON d.hadm_id = adm.hadm_id
      WHERE d.icd_code = 'Z992'
  ),
  All_PD_Events AS (
      SELECT subject_id, CAST(chartdate AS DATETIME) AS event_time FROM `physionet-data.mimiciv_3_1_hosp.procedures_icd`
      WHERE icd_code = '5498'
      ...
      SELECT i.subject_id, ce.charttime AS event_time FROM `physionet-data.mimiciv_3_1_icu.chartevents` ce
      JOIN `physionet-data.mimiciv_3_1_icu.icustays` i ON ce.stay_id = i.stay_id
      WHERE ce.itemid IN (225810, 225806, 225807, 227639, 225959, 225951, 225952, 225961, 225953, 225963, 225965, 227638, 227640)
  ),
  All_Transplant_Events AS (
      SELECT subject_id, CAST(chartdate AS DATETIME) AS event_time FROM `physionet-data.mimiciv_3_1_hosp.procedures_icd`
      WHERE (icd_version = 9 AND icd_code IN ('5569', '5553')) OR (icd_version = 10 AND icd_code IN ('0TY00Z0', '0TY00Z1', '0TY00Z2', '0TY10Z0', '0TY10Z1', '0TY10Z2'))
  )
  SELECT
      bc.hadm_id,
      MAX(IF(hemo.event_time < bc.admittime, 1, 0)) AS history_hemodialysis,
      MAX(IF(pd.event_time < bc.admittime, 1, 0)) AS had_peritoneal_dialysis,
      MAX(IF(transplant.event_time < bc.admittime, 1, 0)) AS history_renal_transplant
  FROM Base_Cohort bc
  LEFT JOIN All_Hemo_Events hemo ON bc.subject_id = hemo.subject_id
  LEFT JOIN All_PD_Events pd ON bc.subject_id = pd.subject_id
  LEFT JOIN All_Transplant_Events transplant ON bc.subject_id = transplant.subject_id
  GROUP BY bc.hadm_id
),
NIV_Flag AS (
  SELECT
...
Baseline_Creatinine AS (
  WITH all_creat AS (
      SELECT p.subject_id, le.charttime, le.valuenum
      FROM `physionet-data.mimiciv_3_1_hosp.patients` p
      JOIN `physionet-data.mimiciv_3_1_hosp.labevents` le ON p.subject_id = le.subject_id
      WHERE le.itemid = 50912 AND le.valuenum IS NOT NULL AND le.valuenum > 0
        AND p.subject_id IN (SELECT subject_id FROM Base_Cohort)
  ),
  creat_by_admission AS (
      SELECT
          bc.hadm_id, bc.admittime, bc.subject_id,
          MIN(IF(cr.charttime BETWEEN DATETIME_SUB(bc.admittime, INTERVAL 12 MONTH) AND DATETIME_SUB(bc.admittime, INTERVAL 24 HOUR), cr.valuenum, NULL)) AS creat_12mo_24hr_prior,
          MIN(IF(cr.charttime < bc.admittime AND cr.charttime >= DATETIME_SUB(bc.admittime, INTERVAL 24 HOUR), cr.valuenum, NULL)) AS creat_24hr_prior
      FROM Base_Cohort bc
      LEFT JOIN all_creat cr ON bc.subject_id = cr.subject_id
      GROUP BY bc.hadm_id, bc.admittime, bc.subject_id
  )
  SELECT
      cba.hadm_id,
      COALESCE(
          cba.creat_12mo_24hr_prior,
          cba.creat_24hr_prior,
          (0.74 - (0.2 * IF(pat.gender = 'F', 1, 0)) + (0.08 * IF(adm.race LIKE '%BLACK%', 1, 0)) + (0.003 * (EXTRACT(YEAR FROM adm.admittime) - pat.anchor_year + pat.anchor_age)))
      ) AS baseline_creatinine
  FROM creat_by_admission cba
  JOIN `physionet-data.mimiciv_3_1_hosp.admissions` adm ON cba.hadm_id = adm.hadm_id
...
Baseline_Features AS (
    ...
    all_events_unioned AS (
        SELECT
            tw.stay_id, fis.hadm_id, ev.charttime, ev.itemid,
            CASE
                WHEN ev.itemid IN (52075, 51133, 53133, 52074, 52069, 52073, 229355, 229359, 229361, 229360) THEN ev.valuenum * 1000
                WHEN ev.itemid IN (51199, 51253) THEN ev.valuenum / 1000
                ELSE ev.valuenum
            END AS valuenum,
            ev.value AS comment_val
        FROM Time_Window tw
        INNER JOIN First_ICU_Stay fis ON tw.stay_id = fis.stay_id
        ...
        SELECT
            tw.stay_id, fis.hadm_id, ev.charttime, ev.itemid,
            CASE
                WHEN ev.itemid IN (52075, 51133, 53133, 52074, 52069, 52073) THEN ev.valuenum * 1000
                ELSE ev.valuenum
            END AS valuenum,
            ev.comments AS comment_val
        FROM Time_Window tw
        INNER JOIN First_ICU_Stay fis ON tw.stay_id = fis.stay_id
        INNER JOIN `physionet-data.mimiciv_3_1_hosp.labevents` ev ON fis.hadm_id = ev.hadm_id
        WHERE ev.charttime BETWEEN tw.window_start AND tw.window_end
    ...
    events_ranked AS (
        SELECT hadm_id, stay_id, charttime, valuenum, comment_val,
            CASE
                WHEN itemid IN (226707) THEN 'height_in'
                WHEN itemid IN (226730) THEN 'height_cm'
                WHEN itemid IN (226531) THEN 'weight_lbs'
                WHEN itemid IN (226512) THEN 'weight_kg'
                WHEN itemid IN (229770) THEN 'resting_pulse'
                WHEN itemid IN (220045) THEN 'heart_rate'
                WHEN itemid IN (220210, 224690) THEN 'resp_rate'
                WHEN itemid IN (220050, 220179, 225309) THEN 'sbp'
                WHEN itemid IN (220051, 220180, 225310) THEN 'dbp'
                WHEN itemid IN (220052, 220181, 225312) THEN 'mbp'
                WHEN itemid IN (223761) THEN 'temp_f'
                WHEN itemid IN (223762) THEN 'temp_c'
                WHEN itemid IN (226755, 227013) THEN 'gcs'
                WHEN itemid IN (223901) THEN 'gcs_motor'
                WHEN itemid IN (223900) THEN 'gcs_verbal'
                WHEN itemid IN (220739) THEN 'gcs_eye'
                WHEN itemid IN (50811, 51640, 51222) THEN 'hb'
                WHEN itemid IN (51279, 52170) THEN 'rbc'
                WHEN itemid IN (50810) THEN 'hematocrit_calc'
                WHEN itemid IN (52028, 51638, 51639, 51221) THEN 'hematocrit'
                WHEN itemid IN (51300, 51301, 51755, 220546) THEN 'wbc'
                WHEN itemid IN (51248) THEN 'mch'
                WHEN itemid IN (51249) THEN 'mchc'
                WHEN itemid IN (51691, 51250) THEN 'mcv'
                WHEN itemid IN (52172) THEN 'rdw_sd'
                WHEN itemid IN (51277) THEN 'rdw_cv'
                WHEN itemid IN (51704, 51265) THEN 'platelet_count'
                WHEN itemid IN (51240) THEN 'p_lcc'
                WHEN itemid IN (51256, 225643) THEN 'neutrophils_pct'
                WHEN itemid IN (52075, 229355) THEN 'abs_neutrophil'
                WHEN itemid IN (51244, 51245, 51690, 225641) THEN 'lymphocytes_pct'
                WHEN itemid IN (52769, 51133, 53132, 229358) THEN 'abs_lymphocyte'
                WHEN itemid IN (51254, 225642) THEN 'monocyte_pct'
                WHEN itemid IN (51253, 52074, 229359) THEN 'abs_monocyte'
                WHEN itemid IN (51146, 225639) THEN 'basophil_pct'
                WHEN itemid IN (229361, 52069) THEN 'abs_basophil'
                WHEN itemid IN (51200, 225640) THEN 'eosinophil_pct'
                WHEN itemid IN (51199, 52073, 229360) THEN 'abs_eosinophil'
                WHEN itemid IN (50889, 51652, 227444, 220612) THEN 'crp'
                WHEN itemid IN (51288, 225683, 227470) THEN 'esr'
                WHEN itemid IN (52921, 51274) THEN 'pt'
                WHEN itemid IN (51275, 52923) THEN 'aptt'
                WHEN itemid IN (51675, 51237) THEN 'inr'
                WHEN itemid IN (50931, 50809, 52569, 52027) THEN 'glucose_lab'
                WHEN itemid IN (50854, 51631, 50852) THEN 'hba1c'
                WHEN itemid IN (50907, 50906, 220603) THEN 'total_cholesterol'
                WHEN itemid IN (50905, 50906, 225670, 225671, 227441) THEN 'ldl'
                WHEN itemid IN (50904, 220624) THEN 'hdl'
                WHEN itemid IN (51000, 225693) THEN 'triglycerides'
                WHEN itemid IN (50885, 53089, 225690) THEN 'bilirubin_total'
                WHEN itemid IN (50883, 51592) THEN 'bilirubin_direct'
                WHEN itemid IN (50884, 51751) THEN 'bilirubin_indirect'
                WHEN itemid IN (50878) THEN 'ast'
                WHEN itemid IN (50861) THEN 'alt'
                WHEN itemid IN (53086, 50863) THEN 'alp'
                WHEN itemid IN (50927, 53093) THEN 'ggt'
                WHEN itemid IN (50912, 52546, 52024) THEN 'creatinine'
                WHEN itemid IN (51842, 52647, 51006) THEN 'bun'
                WHEN itemid IN (50920, 52026, 51770) THEN 'egfr'
                WHEN itemid IN (50983, 52623, 50824, 52455) THEN 'sodium'
                WHEN itemid IN (50971, 50833, 52610, 50822, 52452) THEN 'potassium'
                WHEN itemid IN (50902, 52535, 50806, 52434) THEN 'chloride'
                WHEN itemid IN (50882, 50803, 52039) THEN 'bicarbonate'
                WHEN itemid IN (50808, 51624) THEN 'calcium_free'
                WHEN itemid IN (50893, 52035, 52034) THEN 'calcium_total'
                WHEN itemid IN (50970) THEN 'phosphate'
                WHEN itemid IN (50862, 53085, 52022, 53138, 227456, 220574) THEN 'albumin'
                WHEN itemid IN (50821, 220224) THEN 'pao2'
                WHEN itemid IN (220277) THEN 'spo2'
                WHEN itemid IN (223835, 50816) THEN 'fio2'
                WHEN itemid IN (229407, 229393, 229405) THEN 'pf_ratio'
                WHEN itemid IN (50818, 220235) THEN 'paco2'
                WHEN itemid IN (50820, 223830) THEN 'ph_arterial'
                WHEN itemid IN (227073) THEN 'anion_gap'
                WHEN itemid IN (224828) THEN 'base_excess'
                WHEN itemid IN (227519) THEN 'urine_output'
                WHEN itemid IN (51994, 51498) THEN 'urine_specific_gravity'
                WHEN itemid IN (52045, 51094, 51491) THEN 'urine_ph'
                WHEN itemid IN (52044, 51093) THEN 'urine_osmolality'
                WHEN itemid IN (51102) THEN 'urine_total_protein'
                WHEN itemid IN (51069, 52703, 220862) THEN 'urine_albumin'
                WHEN itemid IN (51084, 51981, 51478) THEN 'urine_glucose'
                WHEN itemid IN (51106, 52000, 51082) THEN 'urine_creatinine'
                WHEN itemid IN (51984, 51484) THEN 'urine_ketone'
            END AS concept,
            ROW_NUMBER() OVER (PARTITION BY hadm_id,
                CASE
                    ...
                    WHEN itemid IN (223761) THEN 'temp_f' WHEN itemid IN (223762) THEN 'temp_c' WHEN itemid IN (226755, 227013) THEN 'gcs' WHEN itemid IN (223901) THEN 'gcs_motor' WHEN itemid IN (223900) THEN 'gcs_verbal' WHEN itemid IN (220739) THEN 'gcs_eye'
                    ...
                    WHEN itemid IN (51256, 225643) THEN 'neutrophils_pct' WHEN itemid IN (52075, 229355) THEN 'abs_neutrophil'
                    WHEN itemid IN (51244, 51245, 51690, 225641) THEN 'lymphocytes_pct' WHEN itemid IN (52769, 51133, 53132, 229358) THEN 'abs_lymphocyte'
                    WHEN itemid IN (51254, 225642) THEN 'monocyte_pct' WHEN itemid IN (51253, 52074, 229359) THEN 'abs_monocyte'
                    WHEN itemid IN (51146, 225639) THEN 'basophil_pct' WHEN itemid IN (229361, 52069) THEN 'abs_basophil'
                    WHEN itemid IN (51200, 225640) THEN 'eosinophil_pct' WHEN itemid IN (51199, 52073, 229360) THEN 'abs_eosinophil'
                    WHEN itemid IN (50889, 51652, 227444, 220612) THEN 'crp' WHEN itemid IN (51288, 225683, 227470) THEN 'esr'
                    WHEN itemid IN (52921, 51274) THEN 'pt' WHEN itemid IN (51275, 52923) THEN 'aptt' WHEN itemid IN (51675, 51237) THEN 'inr'
                    WHEN itemid IN (50931, 50809, 52569, 52027) THEN 'glucose_lab' WHEN itemid IN (50854, 51631, 50852) THEN 'hba1c'
                    WHEN itemid IN (50907, 50906, 220603) THEN 'total_cholesterol' WHEN itemid IN (50905, 50906, 225670, 225671, 227441) THEN 'ldl'
                    WHEN itemid IN (50904, 220624) THEN 'hdl' WHEN itemid IN (51000, 225693) THEN 'triglycerides'
                    WHEN itemid IN (50885, 53089, 225690) THEN 'bilirubin_total' WHEN itemid IN (50883, 51592) THEN 'bilirubin_direct'
                    WHEN itemid IN (50884, 51751) THEN 'bilirubin_indirect' WHEN itemid IN (50878) THEN 'ast' WHEN itemid IN (50861) THEN 'alt'
                    WHEN itemid IN (53086, 50863) THEN 'alp' WHEN itemid IN (50927, 53093) THEN 'ggt'
                    WHEN itemid IN (50912, 52546, 52024) THEN 'creatinine' WHEN itemid IN (51842, 52647, 51006) THEN 'bun' WHEN itemid IN (50920, 52026, 51770) THEN 'egfr'
                    WHEN itemid IN (50983, 52623, 50824, 52455) THEN 'sodium' WHEN itemid IN (50971, 50833, 52610, 50822, 52452) THEN 'potassium'
                    WHEN itemid IN (50902, 52535, 50806, 52434) THEN 'chloride' WHEN itemid IN (50882, 50803, 52039) THEN 'bicarbonate'
                    ...
                    WHEN itemid IN (50862, 53085, 52022, 53138, 227456, 220574) THEN 'albumin'
                    WHEN itemid IN (50821, 220224) THEN 'pao2' WHEN itemid IN (220277) THEN 'spo2' WHEN itemid IN (223835, 50816) THEN 'fio2'
                    WHEN itemid IN (229407, 229393, 229405) THEN 'pf_ratio' WHEN itemid IN (50818, 220235) THEN 'paco2' WHEN itemid IN (50820, 223830) THEN 'ph_arterial'
                    WHEN itemid IN (227073) THEN 'anion_gap' WHEN itemid IN (224828) THEN 'base_excess'
                    WHEN itemid IN (227519) THEN 'urine_output' WHEN itemid IN (51994, 51498) THEN 'urine_specific_gravity'
                    WHEN itemid IN (52045, 51094, 51491) THEN 'urine_ph' WHEN itemid IN (52044, 51093) THEN 'urine_osmolality'
                    WHEN itemid IN (51102) THEN 'urine_total_protein' WHEN itemid IN (51069, 52703, 220862) THEN 'urine_albumin'
                    WHEN itemid IN (51084, 51981, 51478) THEN 'urine_glucose' WHEN itemid IN (51106, 52000, 51082) THEN 'urine_creatinine'
                    WHEN itemid IN (51984, 51484) THEN 'urine_ketone'
                END
    ...
    First_Day_Values AS (
        SELECT hadm_id,
            MAX(CASE WHEN concept = 'height_in' THEN valuenum END) AS first_height_in, MAX(CASE WHEN concept = 'height_cm' THEN valuenum END) AS first_height_cm,
            MAX(CASE WHEN concept = 'weight_lbs' THEN valuenum END) AS first_weight_lbs, MAX(CASE WHEN concept = 'weight_kg' THEN valuenum END) AS first_weight_kg,
            ...
            MAX(CASE WHEN concept = 'resp_rate' THEN valuenum END) AS first_resp_rate, MAX(CASE WHEN concept = 'sbp' THEN valuenum END) AS first_sbp,
            MAX(CASE WHEN concept = 'dbp' THEN valuenum END) AS first_dbp, MAX(CASE WHEN concept = 'mbp' THEN valuenum END) AS first_mbp,
            MAX(CASE WHEN concept = 'temp_f' THEN valuenum END) AS first_temp_f, MAX(CASE WHEN concept = 'temp_c' THEN valuenum END) AS first_temp_c,
            MAX(CASE WHEN concept = 'gcs' THEN valuenum END) AS first_gcs, MAX(CASE WHEN concept = 'gcs_motor' THEN valuenum END) AS first_gcs_motor,
            MAX(CASE WHEN concept = 'gcs_verbal' THEN valuenum END) AS first_gcs_verbal, MAX(CASE WHEN concept = 'gcs_eye' THEN valuenum END) AS first_gcs_eye,
            MAX(CASE WHEN concept = 'hb' THEN valuenum END) AS first_hb, MAX(CASE WHEN concept = 'rbc' THEN valuenum END) AS first_rbc,
            MAX(CASE WHEN concept = 'hematocrit_calc' THEN valuenum END) AS first_hematocrit_calc, MAX(CASE WHEN concept = 'hematocrit' THEN valuenum END) AS first_hematocrit,
            MAX(CASE WHEN concept = 'wbc' THEN valuenum END) AS first_wbc, MAX(CASE WHEN concept = 'mch' THEN valuenum END) AS first_mch,
            MAX(CASE WHEN concept = 'mchc' THEN valuenum END) AS first_mchc, MAX(CASE WHEN concept = 'mcv' THEN valuenum END) AS first_mcv,
            MAX(CASE WHEN concept = 'rdw_sd' THEN valuenum END) AS first_rdw_sd, MAX(CASE WHEN concept = 'rdw_cv' THEN valuenum END) AS first_rdw_cv,
            MAX(CASE WHEN concept = 'platelet_count' THEN valuenum END) AS first_platelet_count, MAX(CASE WHEN concept = 'p_lcc' THEN valuenum END) AS first_p_lcc,
            MAX(CASE WHEN concept = 'neutrophils_pct' THEN valuenum END) AS first_neutrophils_pct, MAX(CASE WHEN concept = 'abs_neutrophil' THEN valuenum END) AS first_abs_neutrophil,
            MAX(CASE WHEN concept = 'lymphocytes_pct' THEN valuenum END) AS first_lymphocytes_pct, MAX(CASE WHEN concept = 'abs_lymphocyte' THEN valuenum END) AS first_abs_lymphocyte,
            MAX(CASE WHEN concept = 'monocyte_pct' THEN valuenum END) AS first_monocyte_pct, MAX(CASE WHEN concept = 'abs_monocyte' THEN valuenum END) AS first_abs_monocyte,
            MAX(CASE WHEN concept = 'basophil_pct' THEN valuenum END) AS first_basophil_pct, MAX(CASE WHEN concept = 'abs_basophil' THEN valuenum END) AS first_abs_basophil,
            MAX(CASE WHEN concept = 'eosinophil_pct' THEN valuenum END) AS first_eosinophil_pct, MAX(CASE WHEN concept = 'abs_eosinophil' THEN valuenum END) AS first_abs_eosinophil,
            MAX(CASE WHEN concept = 'crp' THEN valuenum END) AS first_crp, MAX(CASE WHEN concept = 'esr' THEN valuenum END) AS first_esr,
            ...
            MAX(CASE WHEN concept = 'glucose_lab' THEN valuenum END) AS first_glucose_lab, MAX(CASE WHEN concept = 'hba1c' THEN valuenum END) AS first_hba1c,
            MAX(CASE WHEN concept = 'total_cholesterol' THEN valuenum END) AS first_total_cholesterol, MAX(CASE WHEN concept = 'ldl' THEN valuenum END) AS first_ldl,
            MAX(CASE WHEN concept = 'hdl' THEN valuenum END) AS first_hdl, MAX(CASE WHEN concept = 'triglycerides' THEN valuenum END) AS first_triglycerides,
            MAX(CASE WHEN concept = 'bilirubin_total' THEN valuenum END) AS first_bilirubin_total, MAX(CASE WHEN concept = 'bilirubin_direct' THEN valuenum END) AS first_bilirubin_direct,
            MAX(CASE WHEN concept = 'bilirubin_indirect' THEN valuenum END) AS first_bilirubin_indirect, MAX(CASE WHEN concept = 'ast' THEN valuenum END) AS first_ast,
            MAX(CASE WHEN concept = 'alt' THEN valuenum END) AS first_alt, MAX(CASE WHEN concept = 'alp' THEN valuenum END) AS first_alp,
            MAX(CASE WHEN concept = 'ggt' THEN valuenum END) AS first_ggt,
            MAX(CASE WHEN concept = 'creatinine' THEN valuenum END) AS first_creatinine, MAX(CASE WHEN concept = 'bun' THEN valuenum END) AS first_bun,
            MAX(CASE WHEN concept = 'egfr' THEN comment_val END) AS first_egfr_comment,
            MAX(CASE WHEN concept = 'sodium' THEN valuenum END) AS first_sodium, MAX(CASE WHEN concept = 'potassium' THEN valuenum END) AS first_potassium,
            MAX(CASE WHEN concept = 'chloride' THEN valuenum END) AS first_chloride, MAX(CASE WHEN concept = 'bicarbonate' THEN valuenum END) AS first_bicarbonate,
            MAX(CASE WHEN concept = 'calcium_free' THEN valuenum END) AS first_calcium_free, MAX(CASE WHEN concept = 'calcium_total' THEN valuenum END) AS first_calcium_total,
            MAX(CASE WHEN concept = 'phosphate' THEN valuenum END) AS first_phosphate,
            MAX(CASE WHEN concept = 'albumin' THEN valuenum END) AS first_albumin,
            MAX(CASE WHEN concept = 'pao2' THEN valuenum END) AS first_pao2, MAX(CASE WHEN concept = 'spo2' THEN valuenum END) AS first_spo2,
            MAX(CASE WHEN concept = 'fio2' THEN valuenum END) AS first_fio2, MAX(CASE WHEN concept = 'pf_ratio' THEN valuenum END) AS first_pf_ratio,
            ...
            MAX(CASE WHEN concept = 'anion_gap' THEN valuenum END) AS anion_gap, MAX(CASE WHEN concept = 'base_excess' THEN valuenum END) AS base_excess,
            MAX(CASE WHEN concept = 'urine_output' THEN valuenum END) AS first_urine_output, MAX(CASE WHEN concept = 'urine_specific_gravity' THEN valuenum END) AS first_urine_specific_gravity,
            MAX(CASE WHEN concept = 'urine_ph' THEN valuenum END) AS first_urine_ph, MAX(CASE WHEN concept = 'urine_osmolality' THEN valuenum END) AS first_urine_osmolality,
            MAX(CASE WHEN concept = 'urine_total_protein' THEN valuenum END) AS first_urine_total_protein, MAX(CASE WHEN concept = 'urine_albumin' THEN valuenum END) AS first_urine_albumin,
            MAX(CASE WHEN concept = 'urine_glucose' THEN valuenum END) AS first_urine_glucose, MAX(CASE WHEN concept = 'urine_creatinine' THEN valuenum END) AS first_urine_creatinine,
            MAX(CASE WHEN concept = 'urine_ketone' THEN valuenum END) AS first_urine_ketone
        FROM events_ranked WHERE rn = 1 GROUP BY hadm_id
    )
    SELECT
        bc.hadm_id, bc.subject_id,
        (SELECT stay_id FROM First_ICU_Stay WHERE hadm_id = bc.hadm_id) as stay_id,
        fdv.* EXCEPT(hadm_id),
        CASE
            WHEN fdv.first_weight_kg > 0 AND fdv.first_height_cm > 0 THEN fdv.first_weight_kg / POWER(fdv.first_height_cm / 100, 2)
            ELSE NULL
        END as bmi,
        mv.min_heart_rate
    FROM Base_Cohort AS bc
    LEFT JOIN First_Day_Values as fdv ON bc.hadm_id = fdv.hadm_id
    LEFT JOIN (
        SELECT hadm_id, MIN(valuenum) as min_heart_rate
        FROM all_events_unioned
        WHERE itemid = 220045
        GROUP BY hadm_id
    ) as mv ON bc.hadm_id = mv.hadm_id
),

History_KRT_Events AS (
  SELECT adm.hadm_id, STRING_AGG(DISTINCT CAST(pe.itemid AS STRING), '; ') AS history_krt_itemid
  FROM `physionet-data.mimiciv_3_1_hosp.admissions` adm JOIN `physionet-data.mimiciv_3_1_icu.procedureevents` pe ON adm.subject_id = pe.subject_id
  WHERE pe.starttime < adm.admittime AND pe.itemid IN (225441, 225802, 225803, 225805, 225809, 225955)
  AND adm.hadm_id IN (SELECT hadm_id FROM Base_Cohort)
  GROUP BY adm.hadm_id
),

Custom_AKI_Aggregated AS (
 SELECT
     tm.stay_id, MAX(GREATEST(COALESCE(cr.aki_stage_creat, 0), COALESCE(uo.aki_stage_uo, 0), COALESCE(crrt.aki_stage_crrt, 0))) AS aki_stage_custom_max
 FROM (SELECT stay_id, charttime FROM (SELECT cr.stay_id, cr.charttime FROM `physionet-data.mimiciv_3_1_derived.kdigo_creatinine` cr) UNION DISTINCT SELECT stay_id, charttime FROM (SELECT uo.stay_id, uo.charttime FROM `physionet-data.mimiciv_3_1_derived.kdigo_uo` uo) UNION DISTINCT SELECT stay_id, charttime FROM (SELECT stay_id, charttime FROM `physionet-data.mimiciv_3_1_derived.crrt` WHERE crrt_mode IS NOT NULL)) tm
 LEFT JOIN (SELECT cr.stay_id, cr.charttime, CASE WHEN cr.creat >= (cr.creat_low_past_7day * 3.0) THEN 3 WHEN cr.creat >= 4 AND (cr.creat_low_past_48hr <= 3.7 OR cr.creat >= (1.5 * cr.creat_low_past_7day)) THEN 3 WHEN cr.creat >= (cr.creat_low_past_7day * 2.0) THEN 2 WHEN cr.creat >= (cr.creat_low_past_48hr + 0.3) THEN 1 WHEN cr.creat >= (cr.creat_low_past_7day * 1.5) THEN 1 ELSE 0 END AS aki_stage_creat FROM `physionet-data.mimiciv_3_1_derived.kdigo_creatinine` cr) cr ON tm.stay_id = cr.stay_id AND tm.charttime = cr.charttime
 LEFT JOIN (SELECT uo.stay_id, uo.charttime, CASE WHEN uo.uo_rt_6hr IS NULL THEN NULL WHEN uo.charttime <= DATETIME_ADD(ie.intime, INTERVAL '6' HOUR) THEN 0 WHEN uo.uo_tm_24hr >= 24 AND uo.uo_rt_24hr < 0.3 THEN 3 WHEN uo.uo_tm_12hr >= 12 AND uo.uo_rt_12hr = 0 THEN 3 WHEN uo.uo_tm_12hr >= 12 AND uo.uo_rt_12hr < 0.5 THEN 2 WHEN uo.uo_tm_6hr >= 6 AND uo.uo_rt_6hr < 0.5 THEN 1 ELSE 0 END AS aki_stage_uo FROM `physionet-data.mimiciv_3_1_derived.kdigo_uo` uo INNER JOIN `physionet-data.mimiciv_3_1_icu.icustays` ie ON uo.stay_id = ie.stay_id) uo ON tm.stay_id = uo.stay_id AND tm.charttime = uo.charttime
 LEFT JOIN (SELECT stay_id, charttime, 3 AS aki_stage_crrt FROM `physionet-data.mimiciv_3_1_derived.crrt` WHERE crrt_mode IS NOT NULL) crrt ON tm.stay_id = crrt.stay_id AND tm.charttime = crrt.charttime
 GROUP BY tm.stay_id
),
...
Outcomes_General AS (
  SELECT
      ...
      LEAST(icu.icu_los_days, DATETIME_DIFF(adm.dischtime, adm.admittime, HOUR) / 24.0) AS icu_los_days,
      CASE
  ...
  FROM Base_Cohort bc
  INNER JOIN `physionet-data.mimiciv_3_1_hosp.admissions` AS adm ON bc.hadm_id = adm.hadm_id
  INNER JOIN `physionet-data.mimiciv_3_1_hosp.patients` AS pat ON bc.subject_id = pat.subject_id
  INNER JOIN First_ICU_Stay AS icu ON bc.hadm_id = icu.hadm_id
),
Readmission_Outcome AS (
   WITH next_admission AS (
       SELECT hadm_id, subject_id, dischtime,
...
MAKE_KRT_Outcomes AS (
   WITH all_krt_events AS (
       SELECT hadm_id, subject_id, CAST(chartdate AS DATETIME) as krt_time, 'procedure_icd' as krt_source, icd_code as krt_code
       FROM `physionet-data.mimiciv_3_1_hosp.procedures_icd`
       WHERE icd_code IN ('3995', '5498', '3895', '5A1D70Z', '5A1D80Z', '5A1D90Z')
       UNION ALL
       SELECT i.hadm_id, i.subject_id, pe.starttime as krt_time, 'procedureevents' as krt_source, CAST(pe.itemid AS STRING) as krt_code
       FROM `physionet-data.mimiciv_3_1_icu.procedureevents` pe
       JOIN `physionet-data.mimiciv_3_1_icu.icustays` i ON pe.stay_id = i.stay_id
       WHERE pe.itemid IN (225441, 225802, 225803, 225805, 225809, 225955)
       UNION ALL
       SELECT i.hadm_id, i.subject_id, ce.charttime as krt_time, 'chartevents' as krt_source, CAST(ce.itemid AS STRING) as krt_code
       FROM `physionet-data.mimiciv_3_1_icu.chartevents` ce
       JOIN `physionet-data.mimiciv_3_1_icu.icustays` i ON ce.stay_id = i.stay_id
       WHERE ce.itemid IN (
           226118, 227357, 225725, 226499, 224154, 225183, 227438, 224191,
           228004, 228005, 228006, 224144, 224145, 224149, 224150, 224151,
           224152, 224153, 225810, 225806, 225807, 227639, 225959, 225951,
   ...
   first_krt_event AS (
       SELECT
           ...
           krt.krt_source,
           krt.krt_code,
           ROW_NUMBER() OVER (PARTITION BY bc.hadm_id ORDER BY krt.krt_time ASC) as rn
       FROM Base_Cohort bc
       JOIN First_ICU_Stay icu ON bc.hadm_id = icu.hadm_id
       JOIN `physionet-data.mimiciv_3_1_hosp.admissions` adm ON bc.hadm_id = adm.hadm_id
       JOIN all_krt_events krt ON bc.hadm_id = krt.hadm_id
       LEFT JOIN Patient_History hist ON bc.hadm_id = hist.hadm_id
       WHERE
           IFNULL(hist.history_hemodialysis, 0) = 0 AND IFNULL(hist.had_peritoneal_dialysis, 0) = 0
           AND krt.krt_time >= icu.intime
           AND krt.krt_time <= LEAST(adm.dischtime, DATETIME_ADD(icu.intime, INTERVAL 30 DAY))
   )
   SELECT
       hadm_id,
       1 AS make_krt,
       krt_time AS make_krt_time,
       krt_source AS make_krt_source,
       krt_code as make_krt_code
   FROM first_krt_event
   WHERE rn = 1
),
MAKE_Creatinine_Outcomes AS (
   WITH all_creat_values AS (
       SELECT
           bc.hadm_id,
           le.charttime,
           le.valuenum,
           ROW_NUMBER() OVER(PARTITION BY bc.hadm_id ORDER BY le.charttime DESC) as rn_desc
       FROM Base_Cohort bc
       JOIN First_ICU_Stay icu ON bc.hadm_id = icu.hadm_id
       JOIN `physionet-data.mimiciv_3_1_hosp.admissions` adm ON bc.hadm_id = adm.hadm_id
       JOIN `physionet-data.mimiciv_3_1_hosp.labevents` le ON bc.hadm_id = le.hadm_id
       WHERE le.itemid IN (50912, 52546, 52024)
       AND le.charttime <= LEAST(adm.dischtime, DATETIME_ADD(icu.intime, INTERVAL 30 DAY))
   ),
   max_creat AS (
       SELECT hadm_id, MAX(valuenum) as make_max_creat_value, MAX(charttime) as make_max_creat_value_time
       FROM all_creat_values GROUP BY hadm_id
   ),
   final_creat AS (
       SELECT hadm_id, valuenum as make_final_creat_value
       FROM all_creat_values WHERE rn_desc = 1
   )
   SELECT
       mc.hadm_id,
       mc.make_max_creat_value,
       mc.make_max_creat_value_time,
       fc.make_final_creat_value
   FROM max_creat mc
   LEFT JOIN final_creat fc ON mc.hadm_id = fc.hadm_id
),
Current_Renal_Transplant AS (
   WITH first_transplant AS (
        SELECT
           hadm_id,
           CAST(chartdate AS DATETIME) as transplant_chartdate,
           ROW_NUMBER() OVER(PARTITION BY hadm_id ORDER BY chartdate ASC) as rn
       FROM `physionet-data.mimiciv_3_1_hosp.procedures_icd`
       WHERE
           hadm_id IN (SELECT hadm_id FROM Base_Cohort)
           AND ( (icd_version = 9 AND icd_code = '5569') OR
                 (icd_version = 10 AND icd_code IN ('0TY00Z0', '0TY00Z1', '0TY00Z2', '0TY10Z0', '0TY10Z1', '0TY10Z2')) )
   )
   SELECT
      hadm_id,
      1 AS renal_transplant_current,
      transplant_chartdate
   FROM first_transplant
   WHERE rn=1
),
...
APACHE_Scores AS (
    WITH FirstScores AS (
        SELECT
            ...
            ce.itemid,
            ce.valuenum,
            ROW_NUMBER() OVER(PARTITION BY i.hadm_id, ce.itemid ORDER BY ce.charttime ASC) as rn
        FROM `physionet-data.mimiciv_3_1_icu.chartevents` ce
        JOIN `physionet-data.mimiciv_3_1_icu.icustays` i ON ce.stay_id = i.stay_id
        WHERE
            i.hadm_id IN (SELECT hadm_id FROM Base_Cohort)
            AND ce.itemid IN (226743, 226991)
            AND ce.valuenum IS NOT NULL
    )
    SELECT
        hadm_id,
        MAX(CASE WHEN itemid = 226743 THEN valuenum ELSE NULL END) AS apache_ii_score,
        MAX(CASE WHEN itemid = 226991 THEN valuenum ELSE NULL END) AS apache_iii_score
    FROM FirstScores
    WHERE rn = 1
    GROUP BY hadm_id
),
...
First_Day_SOFA AS (
    WITH ventilation_events AS (
       ...
       WHERE itemid = 223849 AND value IS NOT NULL
         AND stay_id IN (SELECT stay_id FROM First_ICU_Stay)
    ),
    vasoactive_agent AS (
       ...
       WHERE ie.itemid IN (221906, 221289, 221662, 221653)
         AND ie.stay_id IN (SELECT stay_id FROM First_ICU_Stay)
...
APSIII_Score AS (
    ...
    pa AS (
       ...
       FROM `physionet-data.mimiciv_3_1_derived.bg` bg
       INNER JOIN First_ICU_Stay ie ON bg.hadm_id = ie.hadm_id
       LEFT JOIN ventilation_events vd ON ie.stay_id = vd.stay_id AND bg.charttime = vd.charttime
...
SAPSII_Score AS (
    ...
    surgflag AS (
       ...
       WHERE adm.hadm_id IN (SELECT hadm_id FROM First_ICU_Stay)
    ),
    comorb AS (
       SELECT hadm_id,
       ...
       FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd`
       WHERE hadm_id IN (SELECT hadm_id FROM First_ICU_Stay)
       GROUP BY hadm_id
    ),
    pafi1 AS (
       SELECT co.stay_id, bg.charttime, pao2fio2ratio AS pao2fio2,
           CASE WHEN vd.stay_id IS NOT NULL THEN 1 ELSE 0 END AS vent,
           CASE WHEN cp.stay_id IS NOT NULL THEN 1 ELSE 0 END AS cpap
       FROM First_ICU_Stay co
       JOIN `physionet-data.mimiciv_3_1_icu.icustays` icu ON co.stay_id = icu.stay_id
       LEFT JOIN `physionet-data.mimiciv_3_1_derived.bg` bg ON co.hadm_id = bg.hadm_id AND bg.specimen = 'ART.' AND bg.charttime > icu.intime AND bg.charttime <= DATETIME_ADD(icu.intime, INTERVAL '24' HOUR)
...
SELECT
  bc.subject_id, bc.hadm_id, icu.stay_id,
  pat.gender, pat.anchor_age, pat.anchor_year, bc.admittime, adm.dischtime,
  outcomes.death_datetime,
  adm.race,
  outcomes.in_hospital_mortality_flag AS hospital_expire_flag,
  rac.aki_icd_code, preg.pregnancy_icd_codes, con.congenital_anomaly_icd_codes, hem_mal.hematolymphoid_icd_codes, mal.malignancy_icd_codes, aids.aids_icd_codes, dnr.dni_dnr_status,

  kdigo.aki_stage_custom_max,
  IFNULL(hist.history_hemodialysis, 0) AS history_hemodialysis,
  IFNULL(hist.had_peritoneal_dialysis, 0) AS had_peritoneal_dialysis,
  IFNULL(hist.history_renal_transplant, 0) AS history_renal_transplant,
  hkrt.history_krt_itemid,
  IFNULL(niv_flag.had_niv, 0) AS had_niv,
  IFNULL(imv_flag.had_imv, 0) AS had_imv,
  IFNULL(rrt_flag.had_rrt, 0) AS had_rrt,
  bcr.baseline_creatinine,

  IFNULL(flags.has_t1d, 0) AS has_t1d,
  IFNULL(flags.has_t2d, 0) AS has_t2d,
  IFNULL(flags.has_hypertension, 0) AS has_hypertension,
  IFNULL(flags.has_dyslipidemia, 0) AS has_dyslipidemia,
  IFNULL(flags.has_ischemic_heart_disease, 0) AS has_ischemic_heart_disease,
  IFNULL(flags.has_angina, 0) AS has_angina,
  IFNULL(flags.has_coronary_artery_disease, 0) AS has_coronary_artery_disease,
  IFNULL(flags.has_myocardial_infarction, 0) AS has_myocardial_infarction,
  IFNULL(flags.has_heart_failure, 0) AS has_heart_failure,
  IFNULL(flags.has_cardiac_arrest, 0) AS has_cardiac_arrest,
  IFNULL(flags.has_shock, 0) AS has_shock,
  IFNULL(flags.has_cardiogenic_shock, 0) AS has_cardiogenic_shock,
  IFNULL(flags.has_atrial_fibrillation, 0) AS has_atrial_fibrillation,
  IFNULL(flags.has_congenital_heart_defect, 0) AS has_congenital_heart_defect,
  IFNULL(flags.has_infective_endocarditis, 0) AS has_infective_endocarditis,
  IFNULL(flags.has_cerebrovascular_disease, 0) AS has_cerebrovascular_disease,
  IFNULL(flags.has_dvt, 0) AS has_dvt,
  IFNULL(flags.has_pneumonia, 0) AS has_pneumonia,
  IFNULL(flags.has_bacterial_pneumonia, 0) AS has_bacterial_pneumonia,
  IFNULL(flags.has_viral_pneumonia, 0) AS has_viral_pneumonia,
  IFNULL(flags.has_copd, 0) AS has_copd,
  IFNULL(flags.has_pulmonary_embolism, 0) AS has_pulmonary_embolism,
  IFNULL(flags.has_pneumothorax, 0) AS has_pneumothorax,
  IFNULL(flags.has_pulmonary_hypertension, 0) AS has_pulmonary_hypertension,
  IFNULL(flags.has_ards, 0) AS has_ards,
  IFNULL(flags.has_respiratory_failure, 0) AS has_respiratory_failure,
  IFNULL(flags.has_sepsis, 0) AS has_sepsis,
  IFNULL(flags.has_cirrhosis, 0) AS has_cirrhosis,
  IFNULL(flags.has_hepatitis, 0) AS has_hepatitis,
  IFNULL(flags.has_peptic_ulcer_disease, 0) AS has_peptic_ulcer_disease,
  IFNULL(flags.has_ckd, 0) AS has_ckd,
  IFNULL(flags.has_aki_comorbidity, 0) AS has_aki_comorbidity,
  IFNULL(flags.has_hemiplegia, 0) AS has_hemiplegia,
  IFNULL(flags.has_dementia, 0) AS has_dementia,
  IFNULL(flags.has_leukemia, 0) AS has_leukemia,
  IFNULL(flags.has_lymphoma, 0) AS has_lymphoma,
  IFNULL(flags.has_aids, 0) AS has_aids,
  IFNULL(flags.has_connective_tissue_disease, 0) AS has_connective_tissue_disease,
  IFNULL(flags.has_dnr_status, 0) AS has_dnr_status,
  IFNULL(flags.has_rheumatoid_arthritis, 0) AS has_rheumatoid_arthritis,
  ...
  CASE WHEN outcomes.in_hospital_mortality_flag = 1 AND DATETIME_DIFF(outcomes.death_datetime, icu.intime, DAY) <= 28 THEN 1 ELSE 0 END AS icu_acm_28_days,
  CASE WHEN outcomes.in_hospital_mortality_flag = 1 AND DATETIME_DIFF(outcomes.death_datetime, adm.admittime, DAY) < 29 THEN 1 ELSE 0 END AS hospital_acm_28_days,
  CASE WHEN outcomes.death_datetime IS NOT NULL AND DATETIME_DIFF(outcomes.death_datetime, adm.admittime, DAY) <= 91 THEN 1 ELSE 0 END AS mortality_3_months,
  CASE WHEN outcomes.death_datetime IS NOT NULL AND DATETIME_DIFF(outcomes.death_datetime, adm.admittime, DAY) <= 181 THEN 1 ELSE 0 END AS mortality_6_months,
  CASE WHEN outcomes.death_datetime IS NOT NULL AND DATETIME_DIFF(outcomes.death_datetime, adm.admittime, DAY) <= 271 THEN 1 ELSE 0 END AS mortality_9_months,
  CASE WHEN outcomes.death_datetime IS NOT NULL AND DATETIME_DIFF(outcomes.death_datetime, adm.admittime, DAY) <= 366 THEN 1 ELSE 0 END AS mortality_12_months,
  IFNULL(readmit.readmission_30_days, 0) AS readmission_30_days,
  IFNULL(icu_readmit.icu_readmission_30_day, 0) AS icu_readmission_30_day,

  outcomes.in_hospital_mortality_flag AS make_death,
  IFNULL(make_krt.make_krt, 0) AS make_krt,
  make_krt.make_krt_time,
  make_krt.make_krt_source,
  make_krt.make_krt_code,
  make_creat.make_max_creat_value,
  make_creat.make_max_creat_value_time,
  make_creat.make_final_creat_value,

  CASE
      WHEN make_creat.make_final_creat_value >= (2 * bcr.baseline_creatinine) THEN 1
      ELSE 0
  END AS make_persistent_renal_dysfunction,

  outcomes.in_hospital_mortality_duration,
  outcomes.overall_mortality_flag,
  outcomes.overall_mortality_duration,

  IFNULL(crt.renal_transplant_current, 0) AS renal_transplant_current,
  crt.transplant_chartdate AS renal_transplant_chartdate,
  ...
  ci.charlson_comorbidity_index,
  IFNULL(ci.myocardial_infarct, 0) AS charlson_myocardial_infarct,
  IFNULL(ci.congestive_heart_failure, 0) AS charlson_congestive_heart_failure,
  IFNULL(ci.peripheral_vascular_disease, 0) AS charlson_peripheral_vascular_disease,
  IFNULL(ci.cerebrovascular_disease, 0) AS charlson_cerebrovascular_disease,
  IFNULL(ci.dementia, 0) AS charlson_dementia,
  IFNULL(ci.chronic_pulmonary_disease, 0) AS charlson_chronic_pulmonary_disease,
  IFNULL(ci.rheumatic_disease, 0) AS charlson_rheumatic_disease,
  IFNULL(ci.peptic_ulcer_disease, 0) AS charlson_peptic_ulcer_disease,
  IFNULL(ci.mild_liver_disease, 0) AS charlson_mild_liver_disease,
  IFNULL(ci.diabetes_without_cc, 0) AS charlson_diabetes_without_cc,
  IFNULL(ci.diabetes_with_cc, 0) AS charlson_diabetes_with_cc,
  IFNULL(ci.paraplegia, 0) AS charlson_paraplegia,
  IFNULL(ci.renal_disease, 0) AS charlson_renal_disease,
  IFNULL(ci.malignant_cancer, 0) AS charlson_malignant_cancer,
  IFNULL(ci.severe_liver_disease, 0) AS charlson_severe_liver_disease,
  IFNULL(ci.metastatic_solid_tumor, 0) AS charlson_metastatic_solid_tumor,
  IFNULL(ci.aids, 0) AS charlson_aids,
  apache.apache_ii_score,
  apache.apache_iii_score,

  sofa.sofa_score AS first_day_sofa_score,
...
LEFT JOIN Relevant_AKI_Codes AS rac ON bc.hadm_id = rac.hadm_id
...
LEFT JOIN DNI_DNR_Codes AS dnr ON icu.stay_id = dnr.stay_id
LEFT JOIN Comorbidities_Flags AS flags ON bc.hadm_id = flags.hadm_id
LEFT JOIN Supplemental_Oxygen_Flag AS so2_flag ON bc.hadm_id = so2_flag.hadm_id
LEFT JOIN Custom_AKI_Aggregated AS kdigo ON icu.stay_id = kdigo.stay_id
LEFT JOIN Patient_History AS hist ON bc.hadm_id = hist.hadm_id
LEFT JOIN History_KRT_Events AS hkrt ON bc.hadm_id = hkrt.hadm_id
...
LEFT JOIN Baseline_Creatinine AS bcr ON bc.hadm_id = bcr.hadm_id
LEFT JOIN Baseline_Features AS bf ON bc.hadm_id = bf.hadm_id
LEFT JOIN Outcomes_General AS outcomes ON bc.hadm_id = outcomes.hadm_id
LEFT JOIN Readmission_Outcome AS readmit ON bc.hadm_id = readmit.hadm_id
LEFT JOIN ICU_Readmission_Flag AS icu_readmit ON bc.stay_id = icu_readmit.stay_id
LEFT JOIN MAKE_KRT_Outcomes AS make_krt ON bc.hadm_id = make_krt.hadm_id
LEFT JOIN MAKE_Creatinine_Outcomes AS make_creat ON bc.hadm_id = make_creat.hadm_id
LEFT JOIN Current_Renal_Transplant AS crt ON bc.hadm_id = crt.hadm_id
LEFT JOIN Charlson_Index AS ci ON bc.hadm_id = ci.hadm_id
LEFT JOIN APACHE_Scores AS apache ON bc.hadm_id = apache.hadm_id
LEFT JOIN First_Day_SOFA AS sofa ON icu.stay_id = sofa.stay_id
LEFT JOIN APSIII_Score AS apsiii ON icu.stay_id = apsiii.stay_id
LEFT JOIN SAPSII_Score AS sapsii ON icu.stay_id = sapsii.stay_id
LEFT JOIN OASIS_Score AS oasis ON icu.stay_id = oasis.stay_id
